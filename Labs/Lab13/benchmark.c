...
int main (int argc, char **argv)
{
    int i;
	if (argc != 2){
		fprintf(stderr, "Specify size as first command line argument. ex: %s 96\n", argv[0]);
		exit(1);
	}
	...
	if (test_sizes[0] == 0){
		fprintf(stderr, "Could not read problem size from command line. try: %s 96\n", argv[0]);
		exit(2);
	}
